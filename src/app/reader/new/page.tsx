...
function NewReadingContent() {
  ...
  const handleSearch = useCallback(async (supply: string) => {
    ...
    try {
      ...
      setSaveError(e instanceof Error ? e.message : 'Error al buscar suministro')
      setNotFound(true)
    } finally {
  ...
  const handleSave = async () => {
    ...
    const reading = Number(currentReading)
    if (isNaN(reading) || reading < 0 || currentReading.trim() === '') {
    ...
    <div className="flex flex-col gap-6 pb-20">
      ...
      <Card>
        <CardContent className="p-4 flex gap-2">
          ...
          <Button onClick={() => handleSearch(supplyNumber)} disabled={isSearching} size="icon" className="h-12 w-12" aria-label="Buscar suministro">
  ...
  <Button
            ...
            onClick={handleSave}
            disabled={!currentReading || currentReading.trim() === ''}
          >
