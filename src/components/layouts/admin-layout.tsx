...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      <aside className={cn(
        "bg-muni-blue dark:bg-muni-blue-deep text-white flex-shrink-0 flex flex-col transition-all duration-300 ease-in-out relative",
        collapsed ? "w-16" : "w-64"
      )}>
        <div className={cn("p-4 flex items-center", collapsed ? "justify-center" : "justify-between")}>
          ...
          {collapsed && (
            <div className="h-8 w-8 rounded bg-white/20 flex items-center justify-center font-heading font-bold text-sm">
              CE
            </div>
          )}
        </div>
                ...
                {group.items.map((item) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive(item)
                        ? "bg-white/20 dark:bg-white/15 border-l-2 border-white"
                        : "hover:bg-white/10 dark:hover:bg-white/5 border-l-2 border-transparent"
                    )}
      ...
      <main id="main-content" className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 border-b flex items-center justify-between px-8 bg-background">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="h-8 w-8 rounded-full bg-muni-blue dark:bg-muni-blue-deep flex items-center justify-center text-white text-xs font-bold">
              {initials}
            </div>
          </div>
        </header>
