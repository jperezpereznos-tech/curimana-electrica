...
return (
  <div className="min-h-screen bg-muted/20 flex flex-col">
    <header className="h-16 bg-muni-blue dark:bg-muni-blue-deep text-white flex items-center justify-between px-8 shadow-md relative">
      <div className="absolute top-0 left-0 right-0 h-1 bg-muni-gold" />
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={cn(
                ...
                pathname === item.href ? "bg-muni-canopy/25 border-b-2 border-muni-gold" : "hover:bg-muni-canopy/10"
              )}
            >
              <item.icon size={16} />
              {item.name}
      ...
      <div className="flex items-center space-x-3">
        ...
        <Button variant="ghost" size="icon" onClick={() => signOut()} className="text-white hover:bg-muni-canopy/15" aria-label="Cerrar sesión">
          <LogOut size={20} />
        </Button>
      </div>
    </header>
