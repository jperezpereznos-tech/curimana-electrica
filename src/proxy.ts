...
export async function proxy(request: NextRequest) {
  ...
  if (userId && role != null && role !== '') {
    ...
    console.error('[PROXY] Unhandled error in middleware proxy:', e)
    // Clear cookies and redirect to login if it's a protected route, otherwise allow to proceed
    const isProtectedRoute = url.pathname.startsWith('/admin') ||
      url.pathname.startsWith('/cashier') ||
      url.pathname.startsWith('/reader')
    if (isProtectedRoute) {
...
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico|api/|serwist/|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$|sw\\.js$|manifest\\.json).*)',
  ],
}
